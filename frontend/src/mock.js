export const portfolioData = {
  personal: {
    name: "Prathamesh Patil",
    title: "AI/ML Engineer",
    tagline: "Building intelligent systems that solve real-world problems",
    bio: "Specialized in Machine Learning, Deep Learning, and Conversational AI. I transform complex data into actionable insights and build scalable AI solutions for businesses.",
    email: "prathamesh@example.com",
    medium: "https://medium.com/@prathameshece2022",
    linkedin: "https://linkedin.com/in/prathameshpatil",
    github: "https://github.com/prathameshpatil",
    resumeUrl: "/resume.pdf"
  },
  
  projects: [
    {
      id: 1,
      title: "Neural Style Transfer Engine",
      category: "Deep Learning",
      description: "Real-time artistic style transfer using CNN architectures. Implemented custom loss functions achieving 40% faster inference.",
      tech: ["PyTorch", "FastAPI", "Docker", "AWS"],
      year: "2024"
    },
    {
      id: 2,
      title: "Enterprise ChatBot Platform",
      category: "Conversational AI",
      description: "Multi-tenant chatbot system with NLP capabilities. Handles 10K+ concurrent conversations with 95% intent accuracy.",
      tech: ["LangChain", "OpenAI", "Redis", "React"],
      year: "2024"
    },
    {
      id: 3,
      title: "Predictive Maintenance System",
      category: "Machine Learning",
      description: "Time-series forecasting model for industrial equipment. Reduced downtime by 35% through early anomaly detection.",
      tech: ["TensorFlow", "Scikit-learn", "PostgreSQL", "Grafana"],
      year: "2023"
    },
    {
      id: 4,
      title: "Document Intelligence Bot",
      category: "Conversational AI",
      description: "RAG-based chatbot for document querying. Processes PDFs, extracts context, and answers queries with source citations.",
      tech: ["LlamaIndex", "Pinecone", "FastAPI", "React"],
      year: "2023"
    },
    {
      id: 5,
      title: "Computer Vision Pipeline",
      category: "Deep Learning",
      description: "Object detection and tracking system for retail analytics. Achieved 92% mAP on custom dataset.",
      tech: ["YOLOv8", "OpenCV", "ONNX", "Azure"],
      year: "2023"
    },
    {
      id: 6,
      title: "Sentiment Analysis API",
      category: "Machine Learning",
      description: "Multi-lingual sentiment classifier for social media. Processes 1M+ texts daily with sub-100ms latency.",
      tech: ["BERT", "FastAPI", "Redis", "Kubernetes"],
      year: "2022"
    }
  ],
  
  skills: {
    "Machine Learning": ["Scikit-learn", "XGBoost", "LightGBM", "Feature Engineering"],
    "Deep Learning": ["PyTorch", "TensorFlow", "Keras", "Transformers", "CNNs", "RNNs"],
    "NLP & LLMs": ["LangChain", "LlamaIndex", "OpenAI", "Anthropic", "BERT", "GPT"],
    "Backend": ["FastAPI", "Python", "Node.js", "REST APIs", "GraphQL"],
    "Data & Cloud": ["PostgreSQL", "MongoDB", "Redis", "AWS", "Azure", "Docker"],
    "MLOps": ["MLflow", "Kubernetes", "CI/CD", "Model Monitoring"]
  },
  
  experience: [
    {
      id: 1,
      title: "Senior AI/ML Engineer",
      company: "TechCorp AI",
      period: "2023 - Present",
      description: "Leading ML initiatives for enterprise clients. Built and deployed 5+ production ML systems serving millions of users."
    },
    {
      id: 2,
      title: "Machine Learning Engineer",
      company: "DataSolutions Inc",
      period: "2021 - 2023",
      description: "Developed predictive models and chatbot solutions. Improved model performance by 30% through advanced optimization techniques."
    },
    {
      id: 3,
      title: "AI Research Intern",
      company: "Innovation Labs",
      period: "2020 - 2021",
      description: "Researched novel approaches to NLP and computer vision problems. Published findings in internal technical blogs."
    }
  ]
};